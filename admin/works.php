<?php
    ...
    $reqPage = $bdd->query("SELECT * FROM works");
    ...
    if(isset($_GET['delete']) && is_numeric($_GET['delete']))
    {
        ...
        $searchGal = $bdd->prepare("SELECT * FROM images WHERE id_works=?");
        ...
        $deGal = $bdd->prepare("DELETE FROM images WHERE id_works=?");
        $deGal->execute([$id]);


        // supprimer le produit
        $supprimer = $bdd->prepare("DELETE FROM works WHERE id=?");
...
<head>
    ...
    <title>Stock - Administration - Gestion des travaux</title>
</head>
<body>
    <?php
    ...
    <div class="container-fluid">
        ...
        <a href="addWorks.php" class="btn btn-primary my-2">Ajouter une oeuvre</a>
        <?php
            if(isset($_GET['add']) && $_GET['add']=="success")
            {
                echo "<div class='alert alert-success my-2'>Vous avez bien ajouté un nouvelle oeuvre à la base de données</div>";
            }

            if(isset($_GET['update']) && is_numeric($_GET['update']))
            {
                echo "<div class='alert alert-warning my-2'>Vous avez bien modifié le travail #".$_GET['update']."</div>";
            }

            if(isset($_GET['successdelete']) && is_numeric($_GET['successdelete']))
            {
                echo "<div class='alert alert-danger my-2'>Vous avez bien supprimé le travail #".$_GET['successdelete']."</div>";
            }
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php
                    ...
                    $products = $bdd->prepare("SELECT id, name, category, DATE_FORMAT(date,'%d / %m / %Y') AS mydate FROM works LIMIT :offset,:limit");
        ...
        <nav class="d-flex justify-content-center my-5">
            <ul class="pagination">
                <?php
                    if($pg>1)
                    {
                        echo '<li class="page-item"><a href="works.php?page='.($pg-1).'" class="page-link">Previous</a></li>';
                    }
                    ...
                    for($i=1;$i<=$nbPage;$i++)
                    {
                        if($pg==$i)
                        {   
                            echo '<li class="page-item active"><a class="page-link" href="works.php?page='.$i.'">'.$i.'</a></li>';

                        }else{
                            echo '<li class="page-item"><a class="page-link" href="works.php?page='.$i.'">'.$i.'</a></li>';
                        }
                    }

                    if($pg!=$nbPage)
                    {
                        echo '<li class="page-item"><a class="page-link" href="works.php?page='.($pg+1).'">Next</a></li>';
                    }
                ?>   
            </ul>
        </nav>
