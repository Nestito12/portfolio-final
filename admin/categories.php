<?php
    ...
    if(isset($_GET['delete']) && is_numeric($_GET['delete']))
    {
        ...
        $products = $bdd->prepare("SELECT * FROM works WHERE category=?");
        $products->execute([$id]);
        while($donProd = $products->fetch())
        {
            ...
            $galImg = $bdd->prepare("SELECT * FROM images WHERE id_works=?");
        ...
        $delProd = $bdd->prepare("DELETE FROM works WHERE category=?");
        $delProd->execute([$id]);
