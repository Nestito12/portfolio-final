...
if(isset($_POST['nom']))
{
    ...
    if($err == 0)
    {
        ...
        if($_FILES['cover']['error'] == 0)
        {
            ...
            if($errImg == 0)
            {
                ...
                if(move_uploaded_file($_FILES['cover']['tmp_name'],$dossier.$uniqnomSsafe))
                {
                    ...
                    $update = $bdd->prepare("UPDATE works SET name=:nom, date=:date, category=:category, description=:descri, cover=:img WHERE id = :myid");
                    $update->execute([
                        ":nom" => $nom,
                        ":date"=>$date,
                        ":category"=>$categorie,
                    ...
                    header("LOCATION:updateWorks.php?id=".$id."&errorImg=8");
                    exit();
                }

            }else{
                // il y a une erreur personnalisée dans l'upload de l'image
                header("LOCATION:updateWorks.php?id=".$id."&errorImg=".$errImg);
                exit();
            }
        ...
        {
          // update sans image
            $update = $bdd->prepare("UPDATE works SET name=:nom, date=:date, category=:category, description=:descri WHERE id = :myid");
            $update->execute([
                ":nom" => $nom,
                ":date"=>$date,
                ":category"=>$categorie,
        ...
        }else // tu as envoyé une image mais il y a eu une erreur de $_FILES['cover']['error']
        {
            ...
            header("LOCATION:updateWorks.php?id=".$id."&errorImg=".$_FILES['cover']['error']);
            exit();
        }
