            ...
            if($_FILES['cover']['error'] == 0)
            {
                ...
                if($errImg == 0)
                {
                    ...
                    if(move_uploaded_file($_FILES['cover']['tmp_name'],$dossier.$uniqnomSsafe))
                    {
                        ...
                        $insert = $bdd->prepare("INSERT INTO works(name,date,category,description,cover) VALUE(:nom,:date,:category,:descri,:cover)");
                        $insert->execute([
                            ":nom" => $nom,
                            ":date"=>$date,
                            ":category"=>$categorie,
                       ...
                       if($extension == ".jpg")
                       {
                        ...
                        header("LOCATION:addWorks.php?errorImg=8");
                        exit();
                    }

                }else{
                    // il y a une erreur personnalisée dans l'upload de l'image
                    header("LOCATION:addWorks.php?errorImg=".$errImg);
                    exit();
                }
                ...
                header("LOCATION:addWorks.php?errorImg=".$_FILES['cover']['error']);
                exit();
            }
