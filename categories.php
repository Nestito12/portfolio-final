...
?>
    <div class="container">
        ...
        <div class="row">
            ...
            if($mode == "all")
            {
                $req = $bdd->query("SELECT works.cover AS cover, works.name AS pname, categories.name AS cname, DATE_FORMAT(works.date, '%d/%m/%Y') AS mydate, works.id AS pid, categories.id AS cid FROM works INNER JOIN categories ON works.category = categories.id ORDER BY works.date DESC");
            }else{
                $req = $bdd->prepare("SELECT works.cover AS cover, works.name AS pname, categories.name AS cname, DATE_FORMAT(works.date, '%d/%m/%Y') AS mydate, works.id AS pid, categories.id AS cid FROM works INNER JOIN categories ON works.category = categories.id WHERE works.category=? ORDER BY works.date DESC");
                $req->execute([$mode]);
            }
                    ...
                    echo '<div class="col-lg-3 col-md-4 col-sm-6">';
                        echo '<div class="card my-3">';
                            ...
                            echo ' <div class="card-body">';
                                ...
                                echo ' <a href="works.php?id='.$don['pid'].'" class="btn btn-primary">En savoir plus</a>';
                            echo '</div>';
                        echo '</div>';
                    echo '</div>';
                }
